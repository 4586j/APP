...
| 长时间空闲后出现 401 / 弹窗要求重新输入凭据 | 客户端 TCP 连接断开后重连未自动带上 Basic Auth，非密码错误。关闭资源管理器重新打开、或刷新 Z 盘即可恢复；若仍 401，检查 ERP 账号是否被禁用或密码是否已变更 |
| 文件被他人占用，打开时提示只读 | 文件独占锁 TTL 30 分钟，等对方保存或超时后重新打开 |